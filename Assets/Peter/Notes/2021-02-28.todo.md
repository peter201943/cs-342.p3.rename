






# Todo 2021-02-28








## Love Minigame Todo
- [x] Must Appear in Top Level Folder File
- [x] Love Game Folder File
	- [x] Love Game Prefab File
	- [x] Sessions Folder File
		- [x] Session Prefab Instance 00 Prefab File
			- [x] Question Config
			- [x] Answer Config
		- [x] Session Prefab Instance 01 Prefab File
			- [x] Question Config
			- [x] Answer Config
		- [x] Session Prefab Instance 02 Prefab File
			- [x] Question Config
			- [x] Answer Config
		- [x] Session Prefab Instance 03 Prefab File
			- [x] Question Config
			- [x] Answer Config
		- [x] Session Prefab Instance 04 Prefab File
			- [x] Question Config
			- [x] Answer Config
		- [x] Session Prefab Instance 05 Prefab File
			- [x] Question Config
			- [x] Answer Config
		- [x] Session Prefab Instance 06 Prefab File
			- [x] Question Config
			- [x] Answer Config
		- [x] Session Prefab Instance 07 Prefab File
			- [x] Question Config
			- [x] Answer Config
		- [x] Session Prefab Instance 08 Prefab File
			- [x] Question Config
			- [x] Answer Config
		- [x] Session Prefab Instance 09 Prefab File
			- [x] Question Config
			- [x] Answer Config
		- [x] Session Prefab Instance 10 Prefab File
			- [x] Question Config
			- [x] Answer Config
- [ ] Love Game Prefab
	- [x] Love Game Script Component
	- [x] Background Image Child
	- [x] Person Image Child
	- [x] General Config
	- [x] Spawn Difficulty Config
	- [x] Answer Difficulty Config
	- [x] Points Difficulty Config
- [ ] Love Game Script
	- [x] Question Delay Max Time Float
	- [x] Question Delay Min Time Float
	- [x] Question Delay Decrement Rate Float
	- [x] Answer Max Time Float
	- [x] Answer Min Time Float
	- [x] Answer Time Decrement Rate Float
	- [x] Current Love Points Int
	- [x] Start Love Points Int
	- [x] Max Love Points Int
	- [x] Incorrect Answer Void Method
	- [x] Correct Answer Void Method
	- [x] Current Session Object Reference
	- [x] Sessions Prefab Reference List
	- [x] Sessions Queue Object Reference List
	- [x] Game Manager Script Reference
	- [x] Game Over Logic
	- [x] Game Setup Logic
	- [x] Game Score Logic
	- [x] Game Lose Logic
	- [x] Update UI Logic
	- [x] update score logic
	- [x] Spawn Session Logic
	- [x] Session Activation Logic
	- [x] Message Difficulty Update Logic
	- [x] Spawn Difficulty Update Logic
	- [x] Session Update Logic
	- [x] Setup Logic
	- [ ] LATER Pull Apart Script into Separate Components/Prefabs







## Love Mingame Session Todo
- [x] Must Appear in Love Game Folder File
- [x] Session Folder File
	- [x] Session Prefab File
	- [x] Session Script File
- [x] Session Prefab
	- [x] Session Script Component
	- [x] Question Message Text Child
	- [x] Bubble Image Child
	- [x] Answer Prefab Instance 00 Child
	- [x] Answer Prefab Instance 01 Child
	- [x] Answer Prefab Instance 02 Child
	- [x] General Config
- [x] Session Script
	- [x] Love Game Script Reference (Get From Parent Transform)
	- [x] Answers Object Reference List
	- [x] Correct Answers Object Reference List
	- [x] Session TimeOut Void Method
	- [x] Session TimeOut Logic
	- [x] Current Time Float







## Love Minigame Answer Todo
- [x] Must Appear in Love Game Folder File
- [x] Answer Folder File
	- [x] Answer Prefab File
	- [x] Answer Script File
- [x] Answer Prefab
	- [x] Message Text Child
	- [x] Answer Script Component
	- [x] Box Image Child
	- [x] General Config
- [x] Answer Script
	- [x] Session Script Reference
	- [x] OnMouseDown Void Method






## Game Manager Todo
- [x] Must Appear in Top Level Folder File
- [x] Game Manager Folder File
	- [x] Game Manager Script File
	- [x] Game Manager Prefab File
	- [ ] Game Over Text Image File
- [ ] Game Manager Prefab
	- [x] Game Manager Script Component
	- [ ] Invisible Game Over Image Child
	- [x] Clock Prefab Instance Child
	- [x] Money Bar Prefab Instance Child
	- [x] Love Bar Prefab Instance Child
	- [ ] General Config
- [x] Game Manager Script
	- [x] Game Over Object Reference
	- [x] On Game Over Void Method
	- [x] On Round Over Void Method








## Clock Todo
- [x] Must Appear in Game Manager Folder File
- [x] Clock Folder File
	- [x] Clock Face Image File
	- [x] Hour Hand Image File
	- [x] Minute Hand Image File
	- [x] Clock Script File
- [x] Clock Prefab
	- [x] Minute Hand Child
	- [x] Hour Hand Child
	- [x] Clock Script Component
	- [x] General Config
- [ ] Clock Script
	- [x] Game Manager Script Reference
	- [x] Current Time Float
	- [x] Start Time Float
	- [x] Start Void Method
	- [x] On Time Out Void Method
	- [x] Update Void Method
	- [x] Hour Hand Object Reference
	- [x] Minute Hand Object Reference
	- [ ] Update Hour Hand Rotation
	- [ ] Update Minute Hand Rotation







## Love Bar Todo
- [x] Must Appear in Bar Folder File
- [x] Love Bar Folder File
	- [x] Love Bar Prefab File
	- [x] Love Icon Image File
- [x] Bar Prefab Extension
	- [x] Slider Child
		- [x] Pink Background Color Image Child
		- [x] Red Foreground Color Image Child
	- [x] Love Icon Child








## Money Bar Todo
- [x] Must Appear in Bar Folder File
- [x] Money Bar Folder File
	- [x] Money Bar Prefab File
	- [x] Money Icon Image File
- [x] Bar Prefab Extension
	- [x] Slider Child
		- [x] Dark Green Background Color Image Child
		- [x] Green Foreground Color Image Child
	- [x] Money Icon Child








## Bar Todo
- [x] Must Appear in Game Manager Folder File
- [x] Bar Folder File
	- [x] Bar Script File
	- [x] Bar Prefab File
	- [x] Bar Icon Image File
- [x] Bar Prefab
	- [x] Icon Child
	- [x] Slider Child
	- [x] Bar Script Component
- [x] Bar Script
	- [x] Update Score Void Method
	- [x] On Empty Void Method
	- [x] On Full Void Method
	- [x] Game Manager Script Reference





## Main Todo
- [x] Stickler For Paperwork Scene
	- [x] Work Game Prefab Instance
	- [x] Love Game Prefab Instance
	- [x] Game Manager Prefab Instance







## Renames and Fixes
- [x] Rename SampleSceneErjun to SFPW_Main_Erjun_2021-02-28
- [x] Rename SampleSceneAustin to SFPW_Main_Austin_2021-02-28
- [x] Rename SampleScenePeter to SFPW_Main_Peter_2021-02-28
- [x] Rename SampleSceneFinal to SFPW_Main_Final_2021-02-28
- [x] Create WorkGame Folder in all Persons
- [x] Create LoveGame Folder in all Persons
- [x] Create GameManager Folder in all Persons
- [x] Rename All WorkGameThings to WorkGame
- [x] Rename All LoveGameThings to LoveGame
- [x] Convert All LoveGame instances Objects to LoveGame Prefab Instances
- [x] Convert All WorkGame instances Objects to WorkGame Prefab Instances
- [x] Remove Austin Categorized Folder
- [x] Remove Erjun Categorized Folder
- [x] Remove Final Categorized Folder
- [x] Remove Peter Categorized Folder
- [x] Resolve Scripts of Same Name Problem






## Misc Notes
- https://answers.unity.com/questions/8202/how-to-make-a-split-screen-2-cameras-rendering-at.html
- should probably let the scene set the camera
- https://docs.unity3d.com/ScriptReference/MonoBehaviour.OnMouseDown.html
- https://stackoverflow.com/questions/52710442/single-canvas-and-multiple-panels-vs-multiple-canvas-in-unity-3d
- https://docs.unity3d.com/Manual/InstantiatingPrefabs.html
- https://forum.unity.com/threads/quaternion-rotate-point-around-pivot.299216/
- https://answers.unity.com/questions/260100/instantiate-as-a-child-of-the-parent.html






